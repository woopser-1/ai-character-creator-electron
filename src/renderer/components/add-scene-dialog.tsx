...
export function AddSceneDialog({
	...
	const nextIndex = String(sceneCount + 1).padStart(2, "0");
						...
						{chat.messages.length === 0 && (
							<div className="flex flex-col gap-5 py-10">
								<div className="flex items-baseline gap-3">
									...
									<span className="eyebrow text-foreground/55">Brief</span>
								</div>
								<h2 className="-tracking-[0.02em] max-w-[24ch] font-semibold text-[1.875rem] text-foreground leading-[1.05]">
									What <span className="text-primary">scene</span> are we
									drafting?
								</h2>
								<p className="max-w-[58ch] text-[0.9375rem] text-muted-foreground leading-relaxed">
									Describe a scene, or ask the agent to suggest ideas. The
									prompt will follow{" "}
									{getFullName(character.character)}
									's existing voice and the rest of the gallery.
								</p>
							</div>
						)}
