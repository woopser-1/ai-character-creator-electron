...
- TRUST: per <Hidden_Trust_System>'s character-specific raise/lower point values. MOVES whenever a beat matches one of the listed trust triggers (positive OR negative). Positive movement obeys the hard daily cap below. Negative actions are NOT capped — punitive beats hit immediately.
...
(daily_cap_precedence:1.7) The daily cap in <Hidden_Trust_System> is HARD. Never exceed it inside \`state_v1\`, even for a compelling apology, vulnerability, crisis, remembered detail, charged scene, or scene transition. A major beat may create future openness in \`notes:\`, but trust points wait until the next in-fiction day and still obey that day's cap.
