...
const IMAGE_REFRESH_INTERVAL_MS = 24 * 60 * 60 * 1000;
const IMAGE_REFRESH_STARTUP_DELAY_MS = 20_000;
...
app.whenReady().then(() => {
	console.log("[main] starting", {
		...
		debugLlm: process.env.DEBUG_LLM === "1" || process.env.DEBUG_LLM === "true",
	});
	...
	scheduleImageRefresh(mainWindow);
...
function scheduleImageRefresh(window: BrowserWindow): void {
	const run = () => {
		if (window.isDestroyed()) return;
		void refreshAllProfileImages(window).catch((err) => {
			console.warn("[image-refresh] scheduled run failed", err);
		});
	};
	...
	const scheduleNext = (delayMs: number): void => {
		setTimeout(() => {
			run();
			scheduleNext(IMAGE_REFRESH_INTERVAL_MS);
		}, delayMs);
	};

	void getSettings().then((settings) => {
		const last = settings.lastImageRefreshAt
			? Date.parse(settings.lastImageRefreshAt)
			: Number.NaN;
		const elapsed = Number.isNaN(last)
			? Number.POSITIVE_INFINITY
			: Date.now() - last;
		const firstDelay =
			elapsed >= IMAGE_REFRESH_INTERVAL_MS
				? IMAGE_REFRESH_STARTUP_DELAY_MS
				: Math.max(
						IMAGE_REFRESH_STARTUP_DELAY_MS,
						IMAGE_REFRESH_INTERVAL_MS - elapsed,
					);
