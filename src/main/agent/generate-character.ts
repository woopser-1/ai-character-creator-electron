...
> {
	...
	const sectionSchema = z.object({ section: z.string().min(300).max(5000) });
	const sectionJsonSchema = z.toJSONSchema(sectionSchema);

	const subResults = await Promise.all(
		PERSONALITY_LLM_SECTION_IDS.map(async (sectionId) => {
			const baseSystem = buildPersonalitySectionPrompt(
				sectionId,
				difficulty,
				messageLength,
			);
	...
	const parsed = personalityOnlySchema.safeParse({ additionalPersonalityDetails });
	if (!parsed.success) {
		return {
			ok: false,
