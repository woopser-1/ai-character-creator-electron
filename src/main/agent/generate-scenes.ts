...
> {
	const model: string = generationModel;
	...
	const baseSystem = buildSceneGenerationPrompt(
		character,
		imageModel,
		promptCount,
	);
	...
	try {
		...
		const prefix = isAuth ? "AUTH: " : "runModel threw: ";
	...
	if (!parsed.success) {
		const refusal = SCENES_REFUSAL_PATTERN.test(
			result.finalAssistantText ?? "",
		);
		return {
			ok: false,
	...
	if (
		enforcedCount !== undefined &&
		parsed.data.scenes.length !== enforcedCount
	) {
		return {
			ok: false,
...
export async function generateScenes(
	...
	const enforcedCount =
		typeof input.sceneCount === "number"
			? clampSceneCount(input.sceneCount)
			: undefined;
	...
	if (first.refusal) {
		...
		if (superAdmin) {
			console.log(
				`[generate-scenes] refusal detected under super-admin — retrying with reinforced override`,
			);
...
> {
	const model: string = generationModel;
	const baseSystem = buildSingleSceneGenerationPrompt(
		character,
		existingScenes,
		imageModel,
	);
